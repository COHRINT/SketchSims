...
class Softmax:
    ...
    def buildRectangleModel(self, recBounds, steepness=1):
        ...
        B = np.matrix([-1, 0, recBounds[0][0], 1, 0, -recBounds[1]
                       [0], 0, 1, -recBounds[1][1], 0, -1, recBounds[0][1]]).T
    ...
    def buildOrientedRecModel(self, centroid, orient, length, width, steepness=1):
        ...
        points = []
        points = [[centroid[0]+s2, centroid[1]+s1], [centroid[0]+s4, centroid[1]+s3],
                  [centroid[0]-s2, centroid[1]-s1], [centroid[0]-s4, centroid[1]-s3]]
    ...
    def buildPointsModel(self, points, steepness=1):
        ...
        for i in range(0, len(points)):
            ...
            H = np.matrix(
                [[p1[0], p1[1], 1], [p2[0], p2[1], 1], [mid[0], mid[1], 1]])

            Hnull = (self.nullspace(H)).tolist()
            distMed1 = self.distance(
                mid[0]+Hnull[0][0], mid[1]+Hnull[1][0], centroid[0], centroid[1])
            distMed2 = self.distance(
                mid[0]-Hnull[0][0], mid[1]-Hnull[1][0], centroid[0], centroid[1])
    ...
    def buildTriView(self, pose, length=3, steepness=2):
        l = length
        # Without Cutting
        triPoints = [[pose[0], pose[1]], [pose[0]+l*math.cos(2*-0.261799+math.radians(pose[2])), pose[1]+l*math.sin(2*-0.261799+math.radians(
            pose[2]))], [pose[0]+l*math.cos(2*0.261799+math.radians(pose[2])), pose[1]+l*math.sin(2*0.261799+math.radians(pose[2]))]]

        # With Cutting
        lshort = 0.5
        triPoints = [[pose[0]+lshort*math.cos(2*0.261799+math.radians(pose[2])), pose[1]+lshort*math.sin(2*0.261799+math.radians(pose[2]))], [pose[0]+lshort*math.cos(2*-0.261799+math.radians(pose[2])), pose[1]+lshort*math.sin(2*-0.261799+math.radians(
            pose[2]))], [pose[0]+l*math.cos(2*-0.261799+math.radians(pose[2])), pose[1]+l*math.sin(2*-0.261799+math.radians(pose[2]))], [pose[0]+l*math.cos(2*0.261799+math.radians(pose[2])), pose[1]+l*math.sin(2*0.261799+math.radians(pose[2]))]]
    ...
    def Estep(self, weight, bias, prior_mean, prior_var, alpha=0.5, zeta_c=1, softClassNum=0):
        ...
        for c in range(0, len(weight)):
            ...
            yc2[c] = weight[c]*(var + mean*mean)*weight[c] + \
                2*weight[c]*mean*bias[c] + bias[c]**2
...
# PART B #######################################################
            ...
            for c in range(m):
                ...
                y_cs_squared[c] = w[c].T .dot(var_hat + np.outer(mu_hat, mu_hat.T)) .dot(w[c]) \
...
################################################################
    ...
    def runVB(self, prior, softClassNum):
        ...
        for g in prior.Gs:
            ...
            while(count < 100000):
                ...
                count = count+1
                [mean, var, yc, yc2] = self.Estep(
                    weight, bias, g.mean, g.var, alpha, zeta_c, softClassNum=softClassNum)
                [zeta_c, alpha] = self.Mstep(
                    len(weight), yc, yc2, zeta_c, alpha, steps=100)
                logCHat = self.calcCHat(
                    g.mean, g.var, mean, var, alpha, zeta_c, yc, yc2, mod=softClassNum)
                if(abs(prevLogCHat - logCHat) < 0.00001):
                    break
                else:
                    prevLogCHat = logCHat

            post.addG(Gaussian(mean, var, g.weight *
                               np.exp(logCHat).tolist()[0][0]))
    ...
    def plot1D(self, low=0, high=5, res=100, labels=None, vis=True):
        ...
        suma = [0]*len(x)
        softmax = [[0 for i in range(0, len(x))]
        ...
        for i in range(0, len(x)):
            ...
            for j in range(0, len(self.weights)):
                softmax[j][i] = math.exp(
                    self.weights[j]*x[i] + self.bias[j]) / tmp
        if(vis == True):
            for i in range(0, len(self.weights)):
    ...
    def plot2D(self, low=[0, 0], high=[5, 5], labels=None, vis=True, delta=0.1):
        ...
        model = [[[0 for i in range(0, resy)] for j in range(
            0, resx)] for k in range(0, len(self.weights))]
            ...
            for i in range(0, resx):
                ...
                for j in range(0, resy):
                    ...
                    for k in range(0, len(self.weights)):
                        dem += np.exp(self.weights[k][0]*xx +
                                      self.weights[k][1]*yy + self.bias[k])
                    model[m][i][j] = np.exp(
                        self.weights[m][0]*xx + self.weights[m][1]*yy + self.bias[m])/dem

        dom = [[0 for i in range(0, resy)] for j in range(0, resx)]
        for m in range(0, len(self.weights)):
            for i in range(0, resx):
                for j in range(0, resy):
                    dom[i][j] = np.argmax([model[h][i][j]
                                           for h in range(0, len(self.weights))])
        if(vis):
            ...
            for i in range(0, resx):
                ...
                for j in range(0, resy):
                    ...
                    for k in range(0, len(self.weights)):
                        dem += np.exp(self.weights[k][0]*xx +
                                      self.weights[k][1]*yy + self.bias[k])
                    likelihood[i][j] += np.exp(self.weights[m][0]
                                               * xx + self.weights[m][1]*yy + self.bias[m])/dem
    ...
    def lwisUpdate(self, prior, softClass, numSamples, inverse=False):
        ...
        for i in range(0, numSamples):
            if(not inverse):
                w[i] = p.pointEval(
                    x[i])*self.pointEvalND(softClass, x[i])/q.pointEval(x[i])
            else:
                w[i] = p.pointEval(
                    x[i])*(1-self.pointEvalND(softClass, x[i]))/q.pointEval(x[i])
...
def test1DSoftmax():
    ...
    [x1, numApprox] = a.numericalProduct(
        prior, softClass, low=low, high=high, res=res, vis=False)
...
def test2DSoftmax():
    ...
    # Define Likelihood Model
    a = Softmax(weight, bias)
    [x1, y1, dom] = a.plot2D(low=low, high=high, vis=False)

    a.plot2D(low=low, high=high, vis=True)
    ...
    if(MMS):
        axarr[2].set_title(
            'Posterior GM with observation:' + detectLabels[detect])
    else:
...
def testRectangleModel():
    ...
    axarr[0].contourf(x, y, prior.discretize2D(
        low=[0, 0], high=[10, 5], delta=dela))
...
def testGeneralModel():
    ...
    pz.buildGeneralModel(2, 4, [[1, 0], [2, 0], [3, 0]], np.matrix(
        [-1, 1, -1, 1, 1, -1, 0, -1, -1]).T)
    #print('Plotting Observation Model');
    # pz.plot2D(low=[0,0],high=[10,5],vis=True);
...
def testPlot3D():
    ...
    for i in range(1, numClasses):
        boundries.append([i, 0])
    B = np.matrix([-1, -1, 0.5, -1, -1, 1, 0.5, -1, 1, 1, 0.5, -1, 1, -1, 0.5, -
                   1, -1, -1, -0.5, -1, -1, 1, -0.5, -1, 1, 1, -0.5, -1, 1, -1, -0.5, -1]).T

    pz = Softmax()
    pz.buildGeneralModel(dims=dims, numClasses=numClasses,
                         boundries=boundries, B=B, steepness=steep)
...
def testMakeNear():
    ...
    pzOut.buildOrientedRecModel(
        cent, orient, lengthOut, widthOut, steepness=10)
...
def testDiscritization():
    ...
    pz = Softmax()
    pz.buildOrientedRecModel(centroid, orientation,
                             length, width, steepness=steep)
...
def testLWIS():
    ...
    for i in range(0, 100):
        prior.addG(Gaussian([np.random.random()*4-2,
